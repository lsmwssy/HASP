...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        if share_topo:
            ...
            #     self.topology = PersLay() if use_topology else None
            if self.topo_model == "pointnet":
                self.topology = PointNetEncoder(global_feat=True, feature_transform=False, channel=3, he_init=he_init) \
            ...
            if use_topology and topo_layers is not None and topo_layers[0]:
                # if self.topo_model == 'perslay':
                #     self.topology_1 = PersLay()
                # elif self.topo_model == 'pllay':
                #     self.topology_1 = PLLay()
                if self.topo_model == 'pointnet':
                    self.topology_1 = PointNetEncoder(global_feat=True,
                                                 feature_transform=False, channel=3, he_init=he_init)
                else:
            ...
            if use_topology and topo_layers is not None and topo_layers[1]:
                # if self.topo_model == 'perslay':
                #     self.topology_2 = PersLay()
                # elif self.topo_model == 'pllay':
                #     self.topology_2 = PLLay()
                if self.topo_model == 'pointnet':
                    self.topology_2 = PointNetEncoder(global_feat=True,
                                                 feature_transform=False, channel=3, he_init=he_init)
                else:
                    raise ValueError(f"invalid topo models: {self.topo_model}")
            ...
            if use_topology and topo_layers is not None and topo_layers[2]:
                # if self.topo_model == 'perslay':
                #     self.topology_3 = PersLay()
                # elif self.topo_model == 'pllay':
                #     self.topology_3 = PLLay()
                if self.topo_model == 'pointnet':
                    self.topology_3 = PointNetEncoder(global_feat=True,
                                                 feature_transform=False, channel=3, he_init=he_init)
                else:
                    raise ValueError(f"invalid topo models: {self.topo_model}")
            ...
            if use_topology and topo_layers is not None and topo_layers[3]:
                # if self.topo_model == 'perslay':
                #     self.topology_4 = PersLay()
                # elif self.topo_model == 'pllay':
                #     self.topology_4 = PLLay()
                if self.topo_model == 'pointnet':
                    self.topology_4 = PointNetEncoder(global_feat=True,
        ...
        self.linear_1 = nn.Sequential(nn.Linear(2048, 256), nn.Sigmoid())
        self.linear_2 = nn.Sequential(nn.Linear(2048, 512), nn.Sigmoid())
        self.linear_3 = nn.Sequential(nn.Linear(2048, 1024), nn.Sigmoid())
        self.linear_4 = nn.Sequential(nn.Linear(2048, 2048), nn.Sigmoid())
    ...
    def _forward_impl(self, x):
        ...
        if self.share_topo:
            # o = self.topology(x1)[0]
            o = self.topology(x1)
        else:
            topo_feature = [self.topology_1(x1)[0], self.topology_2(x1)[0],
                            self.topology_3(x1)[0], self.topology_4(x1)[0]]
        if self.share_topo:
            x = self.layer1(x)
            ...
            if self.topo_layers[0]:
                # first topology
                o1 = self.linear_1(o)
                scale1 = o1.unsqueeze(2).unsqueeze(3).expand_as(x)
                out1 = x * scale1
                x = x + out1
            ...
            if self.topo_layers[1]:
                o2 = self.linear_2(o)
                scale2 = o2.unsqueeze(2).unsqueeze(3).expand_as(x)
                out2 = x * scale2
                x = x + out2
            ...
            if self.topo_layers[2]:
                o3 = self.linear_3(o)
                scale3 = o3.unsqueeze(2).unsqueeze(3).expand_as(x)
                out3 = x * scale3
                x = x + out3
            ...
            if self.topo_layers[3]:
                o4 = self.linear_4(o)
                scale4 = o4.unsqueeze(2).unsqueeze(3).expand_as(x)
                out4 = x * scale4
                x = x + out4
